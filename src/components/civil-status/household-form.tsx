...
export function HouseholdForm({ arrondissements }: { arrondissements: Option[] }) {
  ...
  async function onSubmit(e: React.FormEvent) {
    ...
    const res = await fetch("/api/households", {
      ...
      body: JSON.stringify({ address, arrondissementId }),
    });
    setLoading(false);
    if (!res.ok) {
  ...
  return (
    <form onSubmit={onSubmit} className="flex flex-wrap items-end gap-3 rounded-lg border border-[var(--color-border)] bg-[var(--color-surface)] p-4 shadow-sm">
      <div>
        ...
        <input value={address} onChange={(e) => setAddress(e.target.value)} className="w-64 rounded-md border border-[var(--color-border)] px-2 py-1.5 text-sm" />
      </div>
      <div>
        <label className="mb-1 block text-xs font-medium text-[var(--color-text-muted)]">Arrondissement</label>
        <select value={arrondissementId} onChange={(e) => setArrondissementId(e.target.value)} className="w-48 rounded-md border border-[var(--color-border)] px-2 py-1.5 text-sm">
