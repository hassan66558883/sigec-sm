          ...
          <tbody className="divide-y divide-[var(--color-border)]">
            {tariffs.map((t) => (
              <tr key={t.id}>
                ...
                <td className="px-4 py-2.5 text-xs text-[var(--color-text-muted)]">{t.unit ?? "—"}</td>
                <td className="px-4 py-2.5 text-xs text-[var(--color-text-muted)]">{t.arrondissement?.name ?? "Toute la ville"}</td>
            ...
            {tariffs.length === 0 && (
              <tr>
                <td colSpan={9} className="px-4 py-6 text-center text-[var(--color-text-muted)]">Aucun tarif enregistre.</td>
              </tr>
            )}
          </tbody>
        </table>
